...
/**
 * BEGIN TIC TAC TOE GAME CODE SECTION --- Joel Brigida
 * Source: http://jsfiddle.net/rtoal/ThPEH
 */
...
$(function () {
   ...
   set = function () {
       ...
       if (win(score[turn])) {
           	//alert(turn + " wins!");													// disable old alert and use a modal instead
			  	$("h5").first().html( "Congrats, " + turn + " Wins!" ); 			// insert text into first <h5> tag
         	$("#TicTacToe_Modal").modal('show'); 									// show modal window
           startNewGame();
       } else if (moves === SIZE * SIZE) {
           	//alert("Cat\u2019s game!");												// disable old alert and use modal instead
				$("h5").first().html( "We have a tie! It's a Cat's game!" ); 	// insert text into first <h5> tag
         	$("#TicTacToe_Modal").modal('show'); 									// show modal window
           startNewGame();
       } else {
           turn = turn === "X" ? "O" : "X";
